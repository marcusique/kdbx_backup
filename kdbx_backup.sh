...
tar -czvf ${BACKUP_PATH}/$(date +%Y-%m-%d)_backup.tar.gz ${KDBX_LOCATION}/*.kdbx --transform 's?.*/??g' >>${LOGFILE}
echo $(date +%Y-%m-%d_%H-%M-%S) " - Finished to tar.gz kdbx files "$(date +%Y-%m-%d)_backup.tar.gz >>${LOGFILE}
